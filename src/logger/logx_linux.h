...
// Debug-only macros: compile to no-ops in release mode (zero overhead)
struct LogxNoOp {
    template<typename T> LogxNoOp& operator<<(const T&) { return *this; }
    LogxNoOp& operator<<(std::ostream&(*)(std::ostream&)) { return *this; }
};

#ifdef DEBUG
    #define LOGX_DEBUG Logx::LogStream(LogLevel::TRACE, __FILE__, __LINE__)
#else
    #define LOGX_DEBUG LogxNoOp()
#endif
